...
void writeToShm(SharedBlock* blk, const array<Export_Data, NUM_TRANSDUCERS>& transducers) {
    sem_wait(&blk->mutex);
...
array <Export_Data,NUM_TRANSDUCERS> jsonSearch(const map <string, Json::Value>& catalogue, string const& symbol_id){
    ...
    array <Export_Data,NUM_TRANSDUCERS> transducers {};
...
int main () {
    ...
    array <Export_Data,NUM_TRANSDUCERS> transducers = jsonSearch(catalogue,symbol); // Stores in a NUM_TRANSDUCERS-sized all transducers needed values to export to the driver
