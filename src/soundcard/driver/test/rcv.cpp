...
std::string extractSymbolFromPayload(const std::string& payload) {
	Json::Value message;
...
void zmqListenerThread() {
	...
	while (jsonLive.load()) {
		...
		std::string symbol = extractSymbolFromPayload(payload);
		if (!symbol.empty()) {
	...
	while (true) {
		...
		if (cmd == "exit") {
			...
			if (m == "on" && !jsonLive.load()) {
				jsonLive.store(true);
				listener = std::thread(zmqListenerThread);
				std::cout << "ZeroMQ listener ON.\n";
			} else if (m == "off" && jsonLive.load()) {
				jsonLive.store(false);
				if (listener.joinable()) {
					listener.join();
				}
				std::cout << "ZeroMQ listener OFF.\n";
			}
		} else if (cmd == "status") {
			...
			std::cout << "<phase>  Signal phase.                   \t[0:360] deg\n";
		}
	}
...
void runGUI() {
	...
	GLFWwindow* window = glfwCreateWindow(900, 800, "Multi-Channel Sine Generator (ZeroMQ Receiver)", NULL, NULL);
	if (!window) {
		glfwTerminate();
		return;
	}
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (ImGui::Checkbox("Live ZeroMQ Mode", &jLive)) {
...
int main(int argc, char** argv) {
	...
	for (int i = 1; i < argc; i++) {
		...
		if (arg == "--device" && i + 1 < argc) {
			deviceIdx = std::stoi(argv[++i]);
		} else if (arg == "--auto-device") {
			autoDevice = true;
		} else if (arg == "--mode" && i + 1 < argc) {
			chosenMode = parseMode(argv[++i]);
		} else if (arg == "--help") {
			std::cout << "Usage: " << argv[0] << " [--device IDX | --auto-device] [--mode gui|tui]\n";
			return 0;
		}
	}
	...
	if (paErr != paNoError) {
		std::cerr << "PortAudio init failed: " << Pa_GetErrorText(paErr) << "\n";
		return 1;
	}

	if (deviceIdx < 0 && autoDevice) {
		deviceIdx = findFirstCapableDevice();
	}
	if (deviceIdx < 0) {
		deviceIdx = selectAudioDevice();
	}

	if (deviceIdx < 0 || deviceIdx >= Pa_GetDeviceCount()) {
	...
	if (!deviceInfo || deviceInfo->maxOutputChannels < NUM_CHANNELS) {
		std::cerr << "Selected device does not support " << NUM_CHANNELS << " output channels.\n";
		Pa_Terminate();
		return 1;
	}
	...
	paErr = Pa_OpenStream(&stream, nullptr, &outputParams, SAMPLE_RATE, FRAMES_PER_BUFFER, paNoFlag, audioCallback, nullptr);
	if (paErr != paNoError) {
		std::cerr << "Pa_OpenStream failed: " << Pa_GetErrorText(paErr) << "\n";
		Pa_Terminate();
		return 1;
	}
	...
	if (paErr != paNoError) {
		std::cerr << "Pa_StartStream failed: " << Pa_GetErrorText(paErr) << "\n";
